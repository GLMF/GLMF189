//	@(#)	[MB] cv_re_extended_sub_nl_err_free_all.c	Version 1.1 du 15/10/16 - 
#include  <stdio.h>
#include  <stdlib.h>
#include  <sys/types.h>
#include  <regex.h>
...
{
     char           *_regexp, *_string, _errbuf[256], *_p;
     regex_t         _reg;
     int             _cflags, _eflags, _i, _s, _e, _error, _max_sub;
     size_t          _nmatch;
	...
	for (_max_sub = 1, _p = _regexp; (*_p); _p++) {
		if (*_p == '(') {
			_max_sub++;
		}
	}
