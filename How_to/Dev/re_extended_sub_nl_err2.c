//	@(#)	[MB] cv_re_extended_sub_nl_err.c	Version 1.1 du 15/10/16 - 
#include  <stdio.h>
#include  <stdlib.h>
#include  <sys/types.h>
#include  <regex.h>
...
{
     char           *_regexp, *_string, _errbuf[256];
     regex_t         _reg;
...
//          for (_i = 0; _pmatch[_i].rm_so != -1; _i++) {
          for (_i = 0; _i < _nmatch; _i++) {
               ...
               printf("Correspondance %2d : debut = %2d, fin = %2d, valeur = [%.*s] rm_so = %d\n",
                      _i, _s, _e, _e - _s + 1, _string + _s, _pmatch[_i].rm_so);
          }
     }
     else {
